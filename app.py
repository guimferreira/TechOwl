...
def glossario():
    if request.method == 'POST':
        ...
        if termo == '' or definicao == '':
            return '<h1>Os campos "termo" e "definição" precisam estar preenchidos.</h1>'
        else:
            definicoes[termo] = definicao
        return redirect('/gloss')
    else:
        return render_template('glossario.html', glossario=sorted(definicoes.items(), key=lambda x: x[0].lower()))
